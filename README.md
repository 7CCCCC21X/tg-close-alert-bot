...
- **没有精确锚点时的兜底**（例如功能上线时已错过 15:00）：若 A50 的最新报价来自收盘当天之后的夜盘，就用当天日盘结算价（“昨结”，约 16:30）代替 15:00 锚点并在代理说明里注明；若 A50 收盘后还没进入夜盘，有效价就取收盘价（约 50/50）；完全没有 A50 报价时同样按收盘价计算并注明。下一次 15:00 拿到精确锚点后自动切回。
- **有效价** = 上证收盘 × exp(β × ln(A50 现价 / A50 锚点))，β 默认 0.8（`A50_BETA` 可改）：A50 只有 50 只大盘股，上证的涨跌幅通常比 A50 小。
- **σ**：腾讯 `fqkline` 的 40 根上证日 K，与 1.1% 的先验合成。
