...
机器人自带一个只读网页，显示和 `/prob` 相同的概率，每 10 秒自动刷新，手机上可以直接打开。每张卡片写明目标收盘（如“09-28 15:00 上交所收盘（北京时间）”），下方有逐秒跳动的“⏳ 距收盘”倒计时；倒计时以机器人的服务器时间为准，不受手机时钟误差影响，到点后显示“已到收盘时间，等待收盘价确认”。发 `/web`（已加入菜单）获取链接。
