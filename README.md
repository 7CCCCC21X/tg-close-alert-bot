...
| `/setofficial SHEIN 40 09-17 16:00` | 记录交易所官方收盘价（语法同 `/setclose`，支持批量），`/status` 和提醒里显示“官方收盘价 / 相对官方涨跌”，不参与触发判断 |
| `/pause` | 暂停当前订阅 |
| `/resume` | 恢复当前订阅 |
...
币安日 K 昨收和股票交易所的正式收盘价并不相同。想同时看到两者，用 `/setofficial` 记录官方收盘价，语法和 `/setclose` 完全一样（可批量、可带收盘时间）：
...
之后 `/status` 和每条涨跌提醒会在基准行之外多显示一行，例如 `官方收盘价：74.5｜相对官方：+2.013%｜收盘 09-17 16:00（北京时间）`。官方收盘价只作对照，不参与阈值判断；触发仍以当前模式的基准（币安日 K 或手动参考价）为准。价格需与币安合约显示值同口径。
