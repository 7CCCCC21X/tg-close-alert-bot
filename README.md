...
限制：目标日只跳过周末，不认识交易所假期；代理（币安合约、HL KR200、恒指期货）与结算标的之间有基差，KR200 追踪的是 KOSPI 200 而不是 KOSPI 综合指数；σ 是历史估计，不同估法会让概率差十几个百分点；预测市场若对平盘另有规则，需要自行调整。`PROBABILITY=off` 关闭。
