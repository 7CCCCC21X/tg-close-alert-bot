...
| `/setofficial SHEIN 40 09-17 16:00` | 记录官方收盘价（语法同 `/setclose`，支持批量），`/status` 和提醒里显示“官方收盘价 / 相对官方涨跌”，不参与触发判断 |
| `/setexchange SKHYNIX 258000 KRW 09-17 14:30` | 记录证券交易所收盘价，可带 HKD/CNY/KRW 等货币；非美元货币只展示不算涨跌，不参与触发判断 |
| `/pause` | 暂停当前订阅 |
| `/resume` | 恢复当前订阅 |
...
币安日 K 昨收和股票交易所的正式收盘价并不相同。想同时看到它们，用 `/setofficial` 记录官方收盘价、用 `/setexchange` 记录证券交易所收盘价，语法和 `/setclose` 完全一样（可批量、可带收盘时间）：

```text
/setofficial 2026-09-18
UNITREE 74.5 09-17 16:00
SHEIN 39.8 09-17 16:00
```
...
之后 `/status` 和每条涨跌提醒会在基准行之外多显示对应的行，例如：

```text
官方收盘价：74.5｜相对官方：+2.013%｜收盘 09-17 16:00（北京时间）
证券交易所收盘价：258,000 KRW｜币种不同，不计算涨跌｜收盘 09-17 14:30（北京时间）
```
...
价格后可以带货币代码（USD、USDT、HKD、CNY、CNH、KRW、JPY、TWD、EUR、GBP、SGD、INR），也可以写在首行作为整批默认。带非美元货币时只展示、不计算涨跌；不带货币或 USD/USDT 时按同口径计算“相对官方 / 相对交易所”涨跌。`/setclose` 的手动参考价必须与币安合约同口径，不接受非美元货币。这两类价格只作对照，不参与阈值判断；触发仍以当前模式的基准（币安日 K 或手动参考价）为准。不自动换汇。
