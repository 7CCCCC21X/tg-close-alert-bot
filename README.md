...
规则：每 10 分钟刷新一次；取最近一个已完结交易日的日 K 收盘（当天的 K 线要到收盘后 15 分钟才算完结：上交所/深交所 15:00、港交所 16:00、韩交所 15:30 当地时间）；收盘时间按交易所收盘点换算成北京时间显示；币种按交易所（CNY/HKD/KRW）标注，因此默认只展示不算涨跌。某个来源失败时 `/status` 会显示“自动获取失败（原因）”，其他合约不受影响；用 `/setexchange` 手动记录的当日价格优先于自动值。
